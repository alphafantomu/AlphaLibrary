...
function Install:CreateEngine()
	...
	assert(Http:GetAsync(RawRepository..RawRepository..EngineLua, false) ~= nil, 'Install: "AlphaEngine.lua" has an invalid format');
	local Engine = Create(
		'ModuleScript', 
		{
			Name = 'AlphaEngine',
			Parent = RepStorage,
			Archivable = false,
			Source = Http:GetAsync(RawRepository..RawRepository..EngineLua, false)
		}
	);
